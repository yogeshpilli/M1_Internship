...
getAggregatePredictPolarization <- function(T_) {
  T_agg <- T_ %>%
    group_by(Author) %>%
    summarise(nr_of_posts = n(), text = paste0(Content, collapse = " ")) %>%
    arrange(Author)
...
readRedditData <- function(datafile = "Data/Reddit_Brexit_All.rds") {
  ...
  if (! file.exists(datafile)) {
    ...
    author_preds <- mclapply(X = t, FUN = function(T_) {
      ...
      authors <- getAggregatePredictPolarization(T_ = T_)
      authors$Stance = ifelse(authors$PScore <= -0.20, 1, ifelse(authors$PScore >= 0.25, 0, 2))
