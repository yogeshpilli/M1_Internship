...
brexit1<-  read.csv('./Data/DataPeriodwise/Reddit_brexit_period5.csv')
...
png("PDF_Edge_Grouped_Case1_Case2_Against_period5.png", width = 3200, height = 1800, res = 300)
ggplot(plot.data, aes(x = x, fill = column)) + geom_density(lwd=1.5,alpha = 0.2)+
  ...
  ggtitle("PDF of Edge Homogeneity of Case1and Case2 with Source as AgainstBrexit for Time period 5") +
...
png("PDF_Edge_Grouped_Case1_Case2_Neutral_period5.png", width = 3200, height = 1800, res = 300)
ggplot(plot.data1, aes(x = x, fill = column)) + geom_density(lwd=1.5,alpha = 0.2)+
  ...
  ggtitle("PDF of Edge Homogeneity of Case1and Case2 with Source as Neutral for Time period 5 ") +
  theme(plot.title = element_text(hjust = 0.5))
dev.off()
...
png("PDF_Edge_Grouped_Case1_Case2_PRO_period5.png", width = 3200, height = 1800, res = 300)
ggplot(plot.data2, aes(x = x, fill = column)) + geom_density(lwd=1.5,alpha = 0.2)+
  ...
  ggtitle("PDF of Edge Homogeneity of Case1and Case2 with Source as PRO BREXIT for time period 5 ") +
  theme(plot.title = element_text(hjust = 0.5))
dev.off()
...
png("PDF_Edge_Grouped_Case1_ALL_period5.png", width = 3200, height = 1800, res = 300)
ggplot(data_grouped,aes (EdgeHomogeneity,color=factor(TStance), ..scaled..)) + 
  geom_density(lwd = 2) +
  theme_classic()+
  ggtitle("Scaled Density of Edge Homogeneity of Case1 for all Stances Together for Time Period 5")+
  theme(plot.title = element_text(hjust = 0.5))+
...
png("PDF_Edge_Grouped_Case2_ALL_period5.png", width = 3200, height = 1800, res = 300)
ggplot(data_grouped,aes (EdgeHomogeneity,color=factor(SStance), ..scaled..)) + 
  geom_density(lwd = 2) +
  theme_classic()+
  ggtitle("Scaled Density of Edge Homogeneity of Case2 for all Stances Together for Time Period 5")+
  theme(plot.title = element_text(hjust = 0.5))+
